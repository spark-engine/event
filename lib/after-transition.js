...
function watchTransition() {
    // If a transition event occurs on an element which does not support dataset (like svg in some browsers) ignore it.
  Event.on(document, 'transitionstart', function(event) {
    if(event.target.dataset) event.target.dataset.isTransitioning = true
  })
  Event.on(document, 'transitionend', function(event) {
    // If a transition event occurs on an element which does not support dataset (like svg in some browsers) ignore it.
    if(event.target.dataset) event.target.removeAttribute('data-is-transitioning')
  })
}
...
function afterTransition(el, callback, startTimeout) {
  ...
  if (el.dataset.isTransitioning || el.querySelector('[data-is-transitioning="true"]')) {
