...
function watchAnimation() {
  Event.on(document, 'animationstart', function(event) {
    // If an animation event occurs on an element which does not support dataset (like svg in some browsers) ignore it.
    if(event.target.dataset) event.target.dataset.isAnimating = true
  })
  Event.on(document, 'animationend', function(event) {
    // If an animation event occurs on an element which does not support dataset (like svg in older some browsers) ignore it.
    if(event.target.dataset) event.target.removeAttribute('data-is-animating')
  })
}
...
function afterAnimation(el, callback, startTimeout) {
  ...
  if (el.dataset.isAnimating || el.querySelector('[data-is-animating="true"]')) {
